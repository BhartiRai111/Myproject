...
export default function CashBook() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await accountingReportApi.cashBook(fromDate, toDate);
      setReport(res.data);
    } catch (err) {
      toast.error(parseApiError(err, 'Failed to load Cash Book').message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title="Cash Book"
        description="Every cash receipt and payment, read from the Cash account in the accounting journal."
        actions={
      ...
      <Card>
        <CardContent className="p-0">
          ...
          {!loading && (report?.rows.length ?? 0) === 0 && (
            <EmptyState icon={Wallet} title="No cash activity" description="No cash receipts or payments in this range." />
          )}
        </CardContent>
      </Card>
    </div>

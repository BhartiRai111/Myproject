...
export default function PurchaseOrders() {
  ...
  const load = async () => {
    ...
    try {
      ...
      toast.error(parseApiError(err, 'Failed to load purchase orders').message);
    } finally {
      setLoading(false);
    }
  };
  ...
  }, [page, supplierFilter, statusFilter, fromDate, toDate]);
  ...
  const handleCancelConfirm = async () => {
    ...
    setCancelling(true);
    try {
      await purchaseOrderApi.cancel(cancelTarget.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        description="Orders placed with suppliers before they are billed."
        actions={
        ...
        <CardContent className="flex flex-col gap-3 p-4 lg:flex-row lg:items-center">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:flex lg:shrink-0">
            ...
            <Select
              ...
              onValueChange={(v) => {
                setPage(0);
                setStatusFilter(v === ALL ? '' : (v as PurchaseOrderStatus));
              }}
            >
              <SelectTrigger className="w-full lg:w-40">
              ...
              <TableBody>
                {orders.map((o) => (
                  <TableRow key={o.id}>
                    ...
                    <TableCell>{o.supplier ? o.supplier.name : '-'}</TableCell>
                    <TableCell className="text-right font-medium">{o.totalAmount.toFixed(2)}</TableCell>
                    <TableCell className="text-right">{o.receivedAmount.toFixed(2)}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => navigate(`/purchases/orders/${o.id}`)}>
                            <Eye className="h-4 w-4" /> View
                          </DropdownMenuItem>
                          {(o.status === 'DRAFT' || o.status === 'CONFIRMED' || o.status === 'PARTIALLY_RECEIVED') && (
                            <DropdownMenuItem onClick={() => navigate(`/purchases/orders/${o.id}/edit`)}>
                              <Pencil className="h-4 w-4" /> Edit
                            </DropdownMenuItem>
                          )}
                          {o.status !== 'COMPLETED' && o.status !== 'CANCELLED' && (
                            <DropdownMenuItem onClick={() => navigate(`/purchases/bills/new?orderId=${o.id}`)}>
